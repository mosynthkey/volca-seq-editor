...
assert(!DEVICE_PROFILES.bass.oscillatorMidiIndependent, 'bass oscillators not MIDI-independent');

const keysCutoff = getDeviceProfile('keys').motionParams.find(param => param.key === 'cutoff');
assert(keysCutoff?.cc === 44, 'keys cutoff CC44');

const bassPitch2 = getDeviceProfile('bass').motionParams.find(param => param.key === 'vcoPitch2');
