...
const wrapOffset = (offset: number) => ((Math.round(offset) % NUM_OF_STEPS) + NUM_OF_STEPS) % NUM_OF_STEPS;

export const createMotionPattern = (
  key: MotionPatternKey,
  options: { min?: number; max?: number; cycles?: number; offset?: number; random?: () => number } = {},
): number[] => {
  const { min, max } = normalizeRange(options.min ?? 0, options.max ?? 127);
  const cycles = Math.max(1, Math.min(8, Math.round(options.cycles ?? 1)));
  const offset = wrapOffset(options.offset ?? 0);
  ...
  const values = Array.from({ length: NUM_OF_STEPS }, (_, step) => {
    ...
    switch (key) {
      ...
      case 'triangle': return scale(position < 0.5 ? position * 2 : 2 - position * 2, min, max);
      case 'square': return position < 0.5 ? min : max;
      case 'random': return scale(random(), min, max);
    }
  });

  if (!offset) return values;
  return Array.from({ length: NUM_OF_STEPS }, (_, step) => values[(step + offset) % NUM_OF_STEPS]);
};
